...
const handleBusSelect = (selectedBus) => {
  ...
  const exportToCSV = () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `bus-trips-data-${bus?.name || 'bus'}-${format(new Date(), 'yyyy-MM-dd')}.csv`;
  ...
  const exportToPDF = () => {
    ...
    try {
      ...
      const printContent = `
        ...
        <html>
        <head>
          <title>Bus Trips Report - ${bus?.name || 'Bus'}</title>
          <style>
        ...
        <body>
          <h1>Bus Trips Report - ${bus?.name || 'Bus'}</h1>
          <div class="summary">
            ...
            <p><strong>Bus:</strong> ${bus?.name || 'N/A'} (${bus?.plate || 'N/A'})</p>
          </div>
          <table>
            <thead>
              <tr>
        ...
        <div className="bus-dashboard-daily-sales-card">
            ...
            <table className="bus-dashboard-daily-sales-table">
                ...
                {filteredTrips.length > 0 ? (
                    ...
                    <tr>
                    <td colSpan="7" className="bus-dashboard-no-data">
                        No trip data available for the selected period
                    </td>
                    </tr>
                )}
